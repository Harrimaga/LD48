...
namespace LD48
{
    ...
    public class SpriteRenderer
    {
        ...
        public Vertex[] GetVertices(List<Sprite> drawList)
        {
            ...
            Vertex[] vertices1 = new Vertex[SpriteCount * 4];
            ...
            for (int i = 0; i < SpriteCount; i++)
            {
                ...
                vertices1[i * 4 + 0] = new Vertex(p1.X, p1.Y, s.posZ, s.texX + (float)s.currentFrame / (float)s.frames, 1.0f,
                                                s.color[0], s.color[1], s.color[2], s.color[3], s.texID);
                vertices1[i * 4 + 1] = new Vertex(p2.X, p2.Y, s.posZ, s.texX + (float)s.currentFrame / (float)s.frames, 0.0f,
                                                s.color[0], s.color[1], s.color[2], s.color[3], s.texID);
                vertices1[i * 4 + 2] = new Vertex(p3.X, p3.Y, s.posZ, (float)s.currentFrame / (float)s.frames, 0.0f,
                                                s.color[0], s.color[1], s.color[2], s.color[3], s.texID);
                vertices1[i * 4 + 3] = new Vertex(p4.X, p4.Y, s.posZ, (float)s.currentFrame / (float)s.frames, 1.0f,
                                                s.color[0], s.color[1], s.color[2], s.color[3], s.texID);
            }
            return vertices1;
        }
        ...
        private void initRenderData()
        {
            ...
            GL.BufferData(BufferTarget.ArrayBuffer, _maxVertexCount * 9 * sizeof(float) + sizeof(int), IntPtr.Zero, BufferUsageHint.DynamicDraw);
